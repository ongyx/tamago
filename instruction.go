...
var Table = [256]Instruction{
	...
	// 0x7
	{
		...
		func(c *CPU, v interface{}) {
			...
			c.reg.ClearAllExceptf(Flag.Carry)
		},
	},
	...
	// 0x8
	{
		"LD (u16),SP", 2, 20,
		func(c *CPU, v interface{}) {
			c.WriteShort(c.reg.SP, v.(uint16))
		},
	},
	...
	// 0x17
	{
		...
		func(c *CPU, v interface{}) {
			...
			if c.reg.Hasf(Flag.Carry) {
				carry = 1
			}

			if (c.reg.A & 0x80) != 0 {
				// overflow
				c.reg.Setf(Flag.Carry)
			} else {
				c.reg.Clearf(Flag.Carry)
			}
